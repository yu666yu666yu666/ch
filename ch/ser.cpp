...
void fa(std::string jsonstr,int client){
    if (write(client, jsonstr.c_str(), jsonstr.length()) == -1) {
    ...
    if(reply == nullptr){
        ...
        if(reply->type == REDIS_REPLY_INTEGER){
            if(reply->integer == 1){//zai
                ...
                if (epoll_ctl(epoll_fd, EPOLL_CTL_DEL, client, nullptr) == -1) {
    ...
    if(reply == nullptr){
        ...
        if(reply->type == REDIS_REPLY_INTEGER){
            if(reply->integer == 1){
                ...
                tt.did = p.cid;
                tt.gid = "";
                j = {{"1", tt.state},{"2", tt.did},{"3", tt.gid}};
                jsonstr = j.dump();
                fa(jsonstr,pp.fd2);
                freeReplyObject(reply);
                return;
